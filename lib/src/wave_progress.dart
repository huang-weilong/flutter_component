...
class _WaveProgressState extends State<WaveProgress> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _heightAnimation = Tween(begin: 0.0, end: widget.percent).animate(_heightController)
      ..addListener(() {
        setState(() {});
      });
...
class MyPainter extends CustomPainter {
  ...
  MyPainter({this.percent, this.primaryColor, this.secondaryColor, this.offsetPercent})
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(Offset(canvasWidth / 2, canvasWidth / 2), canvasWidth / 2, Paint()..style = PaintingStyle.stroke);
  }
  ...
  _drawWave(Canvas canvas, Size size, double offset, Color color) {
    ...
    path.quadraticBezierTo(canvasWidth / 4 - offset, waveHeight - amplitude, canvasWidth / 2 - offset, waveHeight);

    path.quadraticBezierTo(canvasWidth * 3 / 4 - offset, waveHeight + amplitude, canvasWidth - offset, waveHeight);

    path.quadraticBezierTo(
        canvasWidth / 4 - offset + canvasWidth, waveHeight - amplitude, canvasWidth / 2 - offset + canvasWidth, waveHeight);

    path.quadraticBezierTo(
        canvasWidth * 3 / 4 - offset + canvasWidth, waveHeight + amplitude, canvasWidth - offset + canvasWidth, waveHeight);

    path.lineTo(canvasWidth - offset + canvasWidth, canvasWidth);
    path.lineTo(0 - offset, canvasWidth);
    path.close();
    canvas.clipRRect(RRect.fromRectXY(rect, canvasWidth / 2, canvasWidth / 2));
